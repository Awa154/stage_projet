...
if($('.datanew').length>0){$('.datanew').DataTable({"bFilter":true,"sDom":'fBtlpi','pagingType':'numbers',"ordering":true,"language":{search:' ',sLengthMenu: 'Afficher _MENU_ par page',searchPlaceholder: "Rechercher...",info: "_START_ - _END_ sur _TOTAL_ éléments",infoEmpty: "Aucune activité en cours",infoFiltered: "(filtré de _MAX_ éléments au total)",emptyTable: "Aucune activité en cours",},initComplete:(settings,json)=>{$('.dataTables_filter').appendTo('#tableSearch');$('.dataTables_filter').appendTo('.search-input');},});}
